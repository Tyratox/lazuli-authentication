...
UserConroller.prototype.getCurrentUser = (request, response, next) => {
	this.models.User
		.findOne({
...
UserConroller.prototype.getUser = (request, response, next) => {
	this.models.User
		.findAll({
			include: [
				{
...
UserConroller.prototype.getUserById = (request, response, next) => {
...
UserConroller.prototype.postUser = (request, response, next) => {
	...
	user
		...
		.then(() => {
			...
			return user.setPermissionsRaw(request.body.user.permissions).then(() => {
...
UserConroller.prototype.putUser = (request, response, next) => {
	if (
		...
		.then(user => {
			...
			if (request.hasPermission("admin.user.write")) {
				...
				if (
					...
					promises2.push(user.setPermissionsRaw(request.body.user.permissions));
				}
...
UserConroller.prototype.deleteUser = (request, response, next) => {
	this.models.User
